...
export function buildGeometry(data: MarketData, size: number): Geometry {
  ...
  const rings = data.years.map((year): RingGeometry => {
    ...
    const rest = Math.max(0.6, gap * 0.025);
...
function drawMonthWedge(
  ...
  gradient.addColorStop(selectedStop, color);
...
export function drawStaticArtwork(
  ...
  rings.forEach((ring) => {
    ...
    fillVariableContour(context, ring, center, colors.ink, ring.startSample, end, 0.72);
  });

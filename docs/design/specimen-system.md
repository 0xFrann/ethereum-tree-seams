...
The paper is a field, not a photograph. Build it from the flat `--paper` color plus three layers at 1–10% opacity: one broad warm/cool drift, one fine deterministic grain, and one Turing pattern. The Turing pattern is a Gray-Scott reaction–diffusion grown live on the GPU by `app/components/PaperPattern.tsx`, one cell per CSS pixel, and drawn only at the crest of each stripe: a hairline in the same weight as the plate's grain, about nine pixels from the next, sharp-edged and never blurred. It is the one texture allowed to move, and the motion is never the growing: the reaction is run to a finished labyrinth before the first frame and faded in whole, and from then on a slow drifting field in the kill rate keeps the finished lines coming apart and re-forming. Under reduced motion the field stands still and the pattern is drawn once. It stops at the plate's bark, which the plate publishes to it, so the rings sit on clean paper. No texture may cross text glyphs at high contrast or be baked into the canvas data layer, and at 200% zoom each should recede rather than resemble compression noise.

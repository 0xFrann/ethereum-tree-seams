...
| A11Y-03 — canvas role/fallback | **Pass** | The keyboard-operated canvas is now a labelled `role="group"` with `aria-roledescription="interactive chart"`, concise key instructions, a current selection in its accessible name, and a textual canvas fallback that directs users to equivalent year/month/event controls. The prior static-image role mismatch is gone. Browser QA confirmed arrow-key selection. **Superseded in part:** the plate has since left the tab sequence (`tabIndex={-1}`, no focus ring) because a ring drawn on the canvas box sat well outside the calendar ring and broke the specimen. Arrow, `Home` and `End` handling moved to a document listener that yields to open sheets, modified keys and focused controls, so the keys work with focus anywhere on the page; the instructions moved into `#rings-instructions` and a visible caption in the plate's bottom margin. |
...
| Canvas semantics | **Pass** | The interactive group has instructions, current selection, fallback text, keyboard support, and complete semantic controls following it. It is deliberately **not** a tab stop and carries no focus ring: the arrow keys are read from the document instead, taught by a visible caption in the plate's margin and by `#rings-instructions`. The caption is withdrawn on touch and below 880px of viewport height, where the keys still work but the teaching does not appear. |

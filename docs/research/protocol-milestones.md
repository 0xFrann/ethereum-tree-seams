...
Last reviewed: 2026-08-21. Scope: Ethereum mainnet genesis through that date. Dates and times are UTC.
Companion data: [`protocol-milestones.json`](./protocol-milestones.json); the records the site ships live in `lib/event-data.mjs`.

## Selection

Frontier genesis is the **origin** of the chronology and is drawn at the pith, not as a knot. Eleven upgrades are drawn as **knots**: Homestead, DAO fork, Byzantium, Constantinople/St. Petersburg, Beacon Chain genesis, London, The Merge, Shapella, Dencun, Pectra, and Fusaka.

This is deliberately not a complete fork history. It keeps the events that changed Ethereum's operating phase, consensus, fee market, scaling model, staking lifecycle, or broadly useful execution behaviour; it leaves out emergency maintenance, difficulty-bomb postponements, preparatory forks subsumed by a larger event, and parameter-only changes. No entry claims that a protocol event caused a market movement.

## Rubric

Each candidate was weighed on protocol impact, historical importance, recognisability, narrative value, and how much room it has on the ring. Exact activation timestamps and block, epoch and slot values come from ethereum.org's maintained fork chronology; Ethereum Foundation announcements and the EIPs corroborate the activation trigger and the protocol content.

## Visible chronology

| Date (UTC) | Event | Activation | Category | Treatment | Why it is visible |
|---|---|---:|---|---|---|
| 2015-07-30 15:26:13 | Frontier genesis | block 0 | genesis | origin | The beginning of Ethereum mainnet and of the tree's chronology. |
...
| 2021-08-05 12:33:42 | London / EIP-1559 | block 12,965,000 | fee market | knot | EIP-1559 replaced the first-price-only fee market with a burned protocol base fee plus a priority fee. |
...
### Frontier genesis — origin, not a knot

Frontier was Ethereum's first live mainnet release, producing genesis block 0 on 2015-07-30 at 15:26:13 UTC. It was deliberately bare-bones and aimed at technical users, but it established the programmable mainnet whose growth the rings represent.
...
- Confidence: high.
- Naming detail: EIP-6953 labels Frontier's activation trigger as block 1, while ethereum.org identifies genesis as block 0. The origin is labelled `2015-07-30` and block 0 is genesis.
...
- Confidence: high.
- Date detail: the announcement predicted "roughly" Pi Day; the exact observed timestamp comes from the maintained chronology.
...
- Confidence: high.
- Naming detail: the knot marks the **fork**, a protocol milestone, not the June exploit of The DAO's application contract that preceded it. The exploit was an application failure, not a protocol one, and the summary text says so.
...
- Confidence: high.
- Naming detail: "Metropolis" was the broader development phase; Byzantium and Constantinople are distinct activations and are not collapsed into one dated event.

### Constantinople / St. Petersburg — one simultaneous knot

Constantinople and St. Petersburg activated together at block 7,280,000 on 2019-02-28 at 19:52:04 UTC. Constantinople added `CREATE2`, bitwise shifting, `EXTCODEHASH`, and an issuance/difficulty adjustment; St. Petersburg simultaneously removed EIP-1283 after the originally planned activation was postponed over a security concern.
...
- Confidence: high.
- Naming detail: one knot carrying both simultaneous fork names; "Constantinople" alone is the recognisable short label, and the note keeps St. Petersburg.
...
- Confidence: high.
- Date detail: the configured `MIN_GENESIS_TIME` was 12:00:00 UTC; the maintained chronology records the first produced block at 12:00:23 UTC. The data uses the observed time. The then-current "eth2" name is deprecated.
...
- Backup: [EF, "London Mainnet Announcement"](https://blog.ethereum.org/2021/07/15/london-mainnet-announcement) — official mainnet block announcement; the [ethereum.org chronology](https://ethereum.org/ethereum-forks/#london) provides the observed time.
- Confidence: high.
- Naming detail: shown as `London / EIP-1559`; London contained other EIPs, so the longer summary does not equate the whole fork with EIP-1559.
...
- Confidence: high.
- Date detail: Bellatrix (epoch 144,896 on 2022-09-06) prepared the consensus layer; Paris was the TTD-triggered execution transition commonly called The Merge. PoW block 15,537,393 crossed TTD and the next block, 15,537,394, was the first post-Merge block.
...
- Confidence: high.
- Naming detail: `Shapella` is the conventional combined name; `Shanghai–Capella` is the layer-specific alias.
...
- Confidence: high.
- Date detail: the announcement rounds activation to 13:55 UTC; the chronology records 13:55:35. The data uses the exact observed timestamp.
...
- Confidence: high.
- Naming detail: `Pectra` is the combined short name for Prague (execution) and Electra (consensus).
...
- Confidence: high.
- Naming detail: official sources use both layer orderings (`Fulu–Osaka` and `Osaka/Fulu`); `Fusaka` is the stable short label.

## Exclusions

| Candidate(s) | Date / activation | Reason for leaving it out |
|---|---|---|
| Frontier Thawing | 2015-09-07; block 200,000 | Important launch stabilisation, but its gas-limit thaw is visually and narratively subordinate to genesis and Homestead. |
| Tangerine Whistle and Spurious Dragon | 2016-10-18 / 2016-11-22; blocks 2,463,000 / 2,675,000 | Material emergency DoS remediation, but two more 2016 knots would overcrowd the ring; not every emergency fork is a headline milestone. |
| Istanbul | 2019-12-08; block 9,069,000 | A meaningful EVM, cryptography, and gas-cost upgrade, but less distinct to a general narrative than the same-year completion of Metropolis. |
| Muir, Arrow, and Gray Glacier | 2020-01-02 / 2021-12-09 / 2022-06-30 | These chiefly postponed the proof-of-work difficulty bomb; they are maintenance steps subsumed by the Merge story. |
| Berlin | 2021-04-15; block 12,244,000 | Access lists and gas repricing mattered technically, but London five months later has far greater fee-market and public significance. |
| Altair and Bellatrix | 2021-10-27 / 2022-09-06; epochs 74,240 / 144,896 | Important consensus upgrades, but preparatory steps represented by Beacon genesis and The Merge. |
| Fusaka BPO1 and BPO2 | 2025-12-09 / 2026-01-07; epochs 412,672 / 419,072 | Preconfigured blob-parameter-only forks changed capacity, not the protocol feature set; including them would blur named milestones and parameter tuning. |
| Glamsterdam | planned Q4 2026; no activation epoch or date as of review | In development and scheduled after the review date. No future knot is drawn; it can be added after an official mainnet activation. |
| Hegotá and later roadmap items | 2027 or later; unscheduled | Outside the date scope and subject to change. |

## Cautions

- **Date precision:** scheduled announcements sometimes state a minute while the maintained chronology supplies the observed second. The data uses the observed timestamp and keeps the announcement as corroboration.
- **Post-Merge block numbers:** upgrades activate by consensus-layer epoch and slot. Execution block numbers are useful cross-references, not the trigger.
- **Merge trigger:** The Merge was triggered by terminal total difficulty, not a preselected block height. The TTD and the first post-Merge block are stored separately.
...
- **Cutoff:** no upgrade after Fusaka had activated by the review date. Glamsterdam was officially described as upcoming for Q4 2026, so it is an exclusion rather than a tentative entry.

## Source policy

Every fact here rests on primary Ethereum sources: Ethereum Foundation protocol announcements, ethereum.org's maintained protocol histories, and final EIPs. Block-explorer links embedded by ethereum.org are corroborating observations, not the basis for inclusion. Every URL in the JSON records its source type, review date, confidence, and any naming or timing caveat.

...
export function aggregateMarketData(rows, options = {}) {
  ...
  for (let year = earliestYear; year <= latestYear; year += 1) {
    ...
    for (let month = 0; month < 12; month += 1) {
      ...
      monthly.push({
        ...
        returnPct: round((monthRows.at(-1).close / monthRows[0].open - 1) * 100, 1), volumeUsd: round(volumeUsd),
    ...
    rawYears.push({
      ...
      annual: {
        ...
        low: round(Math.min(...yearRows.map((row) => row.low)), 2), returnPct: round((last.close / first.open - 1) * 100, 1),
        volumeUsd: round(yearRows.reduce((sum, row) => sum + row.volumeUsd, 0)),
      },
      priceShape, months: monthly,
    });
  }

  const logVolumes = rawYears.flatMap((year) => year.months.map((month) => Math.log10(month.averageDailyVolumeUsd)));
  ...
  return {
    ...
    methodology: {
      ...
      volume: `Monthly average daily reported USD volume, log10-transformed and normalized across the observed ${earliestYear}–${latestYear} Bitstamp period.`,
      caveat: `Ethereum chronology begins at genesis, but Bitstamp observations begin ${earliestRow.date}. The pre-series interval is unpriced; source gaps are disclosed and never forward-filled.`,
    },
